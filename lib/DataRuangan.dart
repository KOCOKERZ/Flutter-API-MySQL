...
class _RuanganState extends State<DataRuangan> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> fetchRuanganData() async {
    try {
      final response = await http.get(
        Uri.parse('http://localhost:85/p12_project_php/ruangan.php'),
      );

      if (response.statusCode == 200) {
        Iterable list = json.decode(response.body);
        List<RuanganData> ruanganList = list.map((model) => RuanganData(
  ...
  void tambahDataRuangan(String? kode_ruangan, String? nama_ruangan, String? kapasitas) {
    setState(() {
      daftarRuangan.add(
        RuanganData(KodeRuangan: kode_ruangan, NamaRuangan: nama_ruangan, Kapasitas: kapasitas),
      );
    });
  }
