...
class _DataDosenState extends State<DataDosen> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> fetchDosenData() async {
    final response = await http.get(
      Uri.parse('http://localhost:85/p12_project_php/dosen.php'),
    );

    if (response.statusCode == 200) {
      Iterable list = json.decode(response.body);
      List<DosenData> dosenList = list.map((model) => DosenData(
        nidn: model['nidn'],
