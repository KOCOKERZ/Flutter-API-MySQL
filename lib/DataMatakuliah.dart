...
class _MatakuliahState extends State<DataMatakuliah> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> fetchMatkulData() async {
    try {
      final response = await http.get(
        Uri.parse('http://localhost:85/p12_project_php/matakuliah.php'),
      );

      if (response.statusCode == 200) {
        Iterable list = json.decode(response.body);
        List<MatakuliahData> matkulList = list.map((model) => MatakuliahData(
  ...
  void tambahDataMatkul(String? kode_matkul, String? namaMatkul, String? sks) {
    setState(() {
      daftarMatakuliah.add(
        MatakuliahData(KodeMatakuliah: kode_matkul, NamaMataKuliah: namaMatkul, SKS: sks),
      );
    });
  }
