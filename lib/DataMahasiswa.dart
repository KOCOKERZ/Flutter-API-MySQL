...
class _DataMahasiswaState extends State<DataMahasiswa> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> fetchMahasiswaData() async {
    final response = await http.get(
      Uri.parse('http://localhost:85/p12_project_php/mahasiswa.php'),
    );

    if (response.statusCode == 200) {
      Iterable list = json.decode(response.body);
      List<MahasiswaData> mahasiswaList = list.map((model) => MahasiswaData(
        npm: model['npm'],
